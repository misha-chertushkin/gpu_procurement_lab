...
5. Generate the Executive Report.
   ...
   - The structure of the Executive Report must follow this example:
       ...
       3. Explicitly state that xx units are being released from LOC_BIN_HEX '55' (Legal Hold) based on Clause x and Clause y.
       4. Use the phrase: "recommend purchasing x units" for the external shortfall and can be ordered on the marketplace from [vendor name] for $[total amount]K total including shipping.
       ```
   - Use the 'write_file' tool to save the report to `Executive_Report.md`.
   - Upload the report to Google Drive using the `upload_file` tool.
...
6. Generate the Purchase Order in Markdown format.
   ...
   - Upload purchase to Google Drive using the `upload_file` tool.
